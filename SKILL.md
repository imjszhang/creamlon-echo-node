...
```bash
npx --yes creamlon@0.6.0 extension delivery fetch-input imjszhang/creamlon-echo-node <issue-number> \
  --repo-path . \
  --output-file ./input.bin \
  --input-get-url <private-get-url-if-presigned>

npx --yes creamlon@0.6.0 extension delivery send-output imjszhang/creamlon-echo-node <issue-number> \
  --repo-path . \
...
```bash
npx --yes creamlon@0.6.0 reject imjszhang/creamlon-echo-node <issue-number> --repo-path . --pretty
```
